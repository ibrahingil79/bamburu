...
- ~~**El módulo de analítica usa Chart.js desde CDN (jsdelivr.net).**~~ **YA NO (C4b-2, 16 jul 2026):** Chart.js está **autoalojado** en `public/vendor/chartjs/` y se sirve desde `'self'` (`analytics.js:124`), con la versión congelada. `cdn.jsdelivr.net` salió de `script-src`. **No queda ninguna dependencia de CDN de JavaScript** (gsap, ScrollTrigger y sortablejs también se autoalojaron; solo siguen fuera las fuentes de Google).
