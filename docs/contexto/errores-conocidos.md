...
## Secretos (leído con sangre, 11-jul-2026)
- 🚨 **NUNCA pases un secreto como ARGUMENTO de un comando bajo `sudo`.** `sudo` registra la **línea de comando entera** en `/var/log/auth.log` y en el journal. Auditando si la clave de Anthropic se había filtrado alguna vez —no se había filtrado NUNCA: 0 en el árbol, 0 en los 5.967 objetos de git, 0 en los logs rotados, 0 en los `.service`— la filtré yo, con un `sudo grep -F "$CLAVE" ...`. Quince líneas con la clave en claro, en `auth.log` y en el journal. Hubo que rotarla.
- **Cómo se hace bien:** el secreto **por fichero** (`grep -f fichero`) o por **stdin** (heredoc), nunca en `argv`. Y **comprueba antes si necesitas `sudo`**: `/etc/bamburu.env` es propiedad de `ubuntu` y `ubuntu` está en el grupo `adm` (lee `/var/log/auth.log`) — en todo esto el `sudo` **no hacía ninguna falta**, y sin él nada se registra.
- **El journal es persistente** (`/var/log/journal`, desde el 17-jun) y **no admite borrado selectivo**: quitar una línea exige destruir TODO el historial. `auth.log` sí se puede filtrar en el sitio (`grep -v -f clave > tmp && cat tmp > auth.log` — con `cat >`, no `sed -i`, o se cambia el inode y rsyslog se queda escribiendo en el fichero borrado). **La respuesta buena a un secreto filtrado es ROTARLO**, no perseguir cada copia.
- **Dónde vive la clave:** `/etc/bamburu.env` (`0600 ubuntu:ubuntu`), variable `ANTHROPIC_API_KEY`. **Un solo sitio.** Se lee por `process.env` (systemd la inyecta con `EnvironmentFile`) y, de respaldo, `core/llm.js` lee ese mismo fichero. No hay clave por negocio en la BD. Para rotarla: sustituir la línea **sin `sudo`** y **en el sitio** (no `sed -i`: renombrar necesita permiso sobre `/etc`, que `ubuntu` no tiene), `sudo systemctl restart bamburu`, y confirmar con una llamada real a DISA.
