...
- ⬜ PENDIENTE (no iniciado) — **DISA · límite de tema:** DISA solo ayuda con el negocio del usuario y el uso de Bamburu; NO explica cómo está construida la plataforma por dentro (arquitectura, código, infraestructura, multi-tenant/aislamiento, BD…). Hoy ante "¿qué es un tenant?" da clase técnica; debe redirigir con educación a su papel. Regla por TEMA, no por lista de palabras. Tarea propia (NO es permisos). No iniciar hasta que se indique.
