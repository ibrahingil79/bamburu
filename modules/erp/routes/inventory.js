  ...
  views.get('/', requirePerm('inventory.read'), c => {
    ...
    const content = `
      ...
      async function loadInv(){
        ...
        document.getElementById('kVal').textContent='${sym}'+prods.reduce((a,b)=>a+(b.price*b.stock),0).toFixed(2);
        filterTable();
      }
      function filterTable(){
        ...
        document.getElementById('invBody').innerHTML=f.length?f.map(function(p){
          ...
          return '<tr><td><strong>'+escHtml(p.name)+'</strong></td><td style="color:var(--muted)">'+escHtml(p.sku||'-')+'</td><td>'+escHtml(p.category_name||'-')+'</td><td><strong style="color:'+(p.stock<5?'#ef4444':'inherit')+'">'+p.stock+'</strong></td><td><span class="badge '+b[st]+'">'+sl[st]+'</span></td><td style="white-space:nowrap">'+acts+'</td></tr>';
        }).join(''):'<tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--muted)">Sin productos físicos</td></tr>';
      }
