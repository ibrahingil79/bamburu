...
export function createPagosRoutes(db) {
  ...
  views.get('/', requirePerm('purchases.read'), c => {
    ...
    const content = `
      ...
      const ESTADO_LABEL = { pendiente:'Pendiente', parcial:'Pagada en parte', vencida:'Vencida' };
      const ESTADO_BADGE = { pendiente:'b-yellow', parcial:'b-blue', vencida:'b-red' };
      let pagosRows = [];
      async function loadPagos(){
        let data;
        try { data = await api('GET','/api/erp/pagos'); } catch(e){ toast(e.message||'Error','err'); return; }
        pagosRows = data.rows || [];
        document.getElementById('pagosTotal').textContent = SYM + Number(data.total||0).toFixed(2);
        document.getElementById('pagosCount').textContent = '· ' + pagosRows.length + ' factura' + (pagosRows.length===1?'':'s') + ' pendiente' + (pagosRows.length===1?'':'s');
        document.getElementById('pagosBody').innerHTML = pagosRows.length ? pagosRows.map(function(r){
          ...
          return '<tr class="frow">'
            ...
            +'<td><button class="btn btn-primary btn-sm" onclick="openPagos('+r.supplier_invoice_id+')">Pagar</button></td>'
            +'</tr>';
