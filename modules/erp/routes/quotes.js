...
function insertItems(db, quoteId, lines) {
  const ins = db.prepare('INSERT INTO quote_items (quote_id, product_id, description, quantity, unit_price, total_price, tax_rate, tax_amount, fiscal_treatment, fiscal_exemption_code, fiscal_non_subject_code, fiscal_reverse_charge, fiscal_legal_text) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)');
  for (const l of lines) {
    ...
    ins.run(quoteId, l.product_id || null, l.description, l.quantity, l.unit_price, base, l.tax_rate, tax, l.fiscal_treatment, l.fiscal_exemption_code, l.fiscal_non_subject_code, l.fiscal_reverse_charge, l.fiscal_legal_text);
  }
}
...
export function convertQuoteSvc(db, id, dest) {
  ...
  const lines = items.map(i => ({ description: i.description, quantity: i.quantity, unit_price: i.unit_price, tax_rate: i.tax_rate, product_id: i.product_id || undefined, fiscal_treatment:i.fiscal_treatment,fiscal_exemption_code:i.fiscal_exemption_code,fiscal_non_subject_code:i.fiscal_non_subject_code,fiscal_reverse_charge:i.fiscal_reverse_charge,fiscal_legal_text:i.fiscal_legal_text }));
...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;border-bottom:3px solid ${accent};padding-bottom:16px">
  ...
  <div style="text-align:right;color:var(--text2);font-size:12px">
    <div>Fecha: <strong style="color:${accent}">${fechaEs(q.date)}</strong></div>
    ${q.valid_until ? `<div>Válido hasta: <strong style="color:${accent}">${fechaEs(q.valid_until)}</strong></div>` : ''}
  </div>
</div>
...
<table style="margin-left:auto;width:320px;border-collapse:collapse">
  ...
  ${taxRows}
  ${irpfRow}
  <tr><td style="padding:10px 12px;font-size:15px;border-top:2px solid ${accent};font-weight:700">TOTAL</td><td style="padding:10px 12px;text-align:right;font-size:15px;border-top:2px solid ${accent};font-weight:700">${dineroEs(q.total, sym)}</td></tr>
</table>
