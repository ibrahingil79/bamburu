...
export function createSettingsRoutes(db, cfg = {}) {
  ...
  // pestaña «✦ DISA construye» y su chat. Se borra entero (551 líneas): no estaba montado —el
  // mount de `/admin/store-settings` seguía comentado en `modules/erp/routes/index.js`— así que
  // ya era código muerto antes de tocarlo. `storeViews` se queda como router vacío: revive la
  // pantalla quien monte su ruta otra vez, con el editor que corresponda cuando la tienda se
  // encienda (D2), sin el chat.
